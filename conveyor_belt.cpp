...
/** 
    Class cb_worker : Describe each workers task on the given slot
**/
class cb_worker {
...
public:
  ...
  /*
    @ has_item : return bool if the current item is slot is present in
                 worker's list.
  */
  ...
  /*
    @ has_final_product : return bool if worker's list contains final product 
  */
  bool has_final_product() {
    return _productcount == 0 && has_item(final_product);
  }
  /*
    @ needs_item : return if the current component is needed  
  */
